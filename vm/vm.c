...
DECODE {
        ...
        CASE(LOAD_MODULE_VAR):
            ...
            PUSH(fun->module->moduleVarValue.datas[READ_SHORT()]);
            LOOP();

        CASE(STORE_MODULE_VAR):
            //栈顶：模块变量值

            fun->module->moduleVarValue.datas[READ_SHORT()] = PEEK();
            LOOP();
        ...
        CASE(AND): {
            ...
            if (VALUE_IS_FALSE(condition) || VALUE_IS_NULL(condition))
                ...
                //若条件为真则继续执行and右边的表达式的计算步骤，丢掉栈顶的条件
                DROP();
            LOOP();
        }
        ...
        CASE(RETURN): {
            ...
            if (curThread->usedFrameNum == 0) {
                ...
                //在主调方线程的栈顶存储被调线程的执行结果
                curThread->esp[-1] = retVal;
            } else {
                //将返回值置于运行时栈栈顶
                stackStart[0] = retVal;
                //回收堆栈：保留除结果所在的slot即stackStart[0]，其他全丢弃
        ...
        CASE(CREATE_CLOSURE): {
            //指令流：2字节待创建闭包的函数在常量表中的索引+函数所用的upvalue数 x 2

            //endCompileUnit已经将闭包函数添加进了常量表
            ObjFun *objFun = VALUE_TO_OBJFUN(fun->constants.datas[READ_SHORT()]);
            ObjClosure *objClosure = newObjClosure(vm, objFun);
            //将创建好的闭包的value结构压到栈顶，后续会有函数如defineMethod从栈底取出
        ...
        CASE(STATIC_METHOD): {
            //指令流：待绑定的方法“名字”在vm->allMethodNames中的2字节索引
            //栈顶：待绑定的类 次栈顶：待绑定的方法
